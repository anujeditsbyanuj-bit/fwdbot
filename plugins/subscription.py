...
async def userstats_command(client, message):
    """Admin command to show overall user & subscription statistics"""
    if not await check_owner_sub(message):
        return
    ...
    try:
        ...
        plan_counts = {'free': 0, 'plus': 0, 'pro': 0, 'infinity': 0}
        lifetime_count = 0
        ...
        async for user in all_users:
            ...
            active_tasks = user.get('active_tasks', {})
            if active_tasks.get('forwarding', 0) > 0:
        ...
        premium_total = sum(v for k, v in plan_counts.items() if k != 'free')

        text = (
            f"<b>📊 {to_small_caps('bot statistics')}</b>\n"
            f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
            f"<b>👥 {to_small_caps('total users')}:</b> {total_users}\n"
            f"<b>💎 {to_small_caps('premium users')}:</b> {premium_total}\n"
            f"<b>♾️ {to_small_caps('lifetime users')}:</b> {lifetime_count}\n"
            f"<b>🚫 {to_small_caps('banned users')}:</b> {banned_count}\n"
            f"<b>🚀 {to_small_caps('users with active tasks')}:</b> {active_tasks_count}\n\n"
            f"<b>{to_small_caps('plan breakdown')}:</b>\n"
            f"🆓 {to_small_caps('free')}: {plan_counts.get('free', 0)}\n"
            f"⭐ {to_small_caps('plus')}: {plan_counts.get('plus', 0)}\n"
            f"💎 {to_small_caps('pro')}: {plan_counts.get('pro', 0)}\n"
            f"♾️ {to_small_caps('infinity')}: {plan_counts.get('infinity', 0)}"
        )

        await message.reply(text)

    except Exception as e:
        logging.error(f"Error in userstats_command: {e}")
        logging.error(traceback.format_exc())
        await message.reply(f"<b>❌ {to_small_caps('error')}:</b> <code>{str(e)}</code>")
