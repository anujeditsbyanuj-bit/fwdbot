...
async def cancel_transfer_command(client, message):
    ...
    Runs in an earlier group than the generic /cancel handler in commands.py
    so a pending transfer takes priority. If there's no pending transfer for
    this user, propagate the message so the generic task-cancel handler in
    commands.py still gets a chance to run (previously both handlers were
    registered in the same default group, so only one of them ever fired).
    """
    user_id = message.from_user.id
